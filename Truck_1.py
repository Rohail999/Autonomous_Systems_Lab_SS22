"""
Truck 1 simulator script based on MQTT
...
role_topic = "/truck/truckRole/1"   #for publishing the data related to role assignment of Truck 1
assigned_role_topic = "/truck/truckRole/assigned"

device_token = "TRUCK_1"        #giving a human readable device ID
role=""
roleDefined = False
...
publishSlave2TimeDelay=14 # publishing frequencies for each role /
...
def handling_payload ():
    ...
    if payloadJson['truckID'] == "ROLE_ASSIGNMENT": # checking the truck role coming from roleassginment file
        role =payloadJson['truck1Role']
        ...
        if role == "SLAVE_1":
            if payloadJson['truckRole'] == "MASTER": # if this truckrole is slave1 and incoming data is from truckrole master then it will extract the ts , speed of master
                print(payloadJson['speed'])
...
def knn_classifier ():
    ...
    for i in range(len(roleDataSet)):
        print(i)
        #calculating Euclidean Distance for getting the distance between dataset point and obsereved truck parameter
    ...
    while True:
        ...
        while(roleDefined == False):
            ...
            if (newPayload == True):
                ...
                newPayload = False
            time.sleep(10)
        print("Role defined to this Truck 1 is %s" %(role))
        ...
        if role == "MASTER":
            ...
            # print(truck_data)
            print("Payload for Truck 1 with Role : %s" % (role))
            print(json_string)
