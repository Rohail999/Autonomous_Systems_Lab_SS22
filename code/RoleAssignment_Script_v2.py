...
"""
# responsible for assiging the role of each truck using mqtt connections

import json
import random
import time
import paho.mqtt.client as mqtt # library for all mqtt connection and exchanging data between cient and broker
...
role_topic_2 = "/truck/truckRole/2"   #for getting the data related to role assignment of Truck 2
role_topic_3 = "/truck/truckRole/3"   #for getting the data related to role assignment of Truck 3
role_topic_4 = "/truck/truckRole/4"   #for getting the data related to role assignment of Truck 4
assigned_role_topic = "/truck/truckRole/assigned" # roles will be published on this topic
...
truckMasterCountList= [0,0,0,0]
truckMasterCountMax=0
truckMasterCountMaxIndex= 0
...
def on_connect(client, userdata, flags, rc): # responsible for connecting client to mqtt broker
...
def on_message(client, userdata, msg): # whenever a payload is publich on the subscribed topic , that payload is fetched by this function
...
try:
    while True:
        ...
        if truckResponseReceived == numberOfTrucks:
            ...
            truckMasterCountList = [truck1MasterCount,truck2MasterCount,truck3MasterCount,truck4MasterCount]
            print("List of truck Master Count in sequence is: %s" %(truckMasterCountList))
            truckMasterCountMax =max(truckMasterCountList)
            truckMasterCountMaxIndex =  truckMasterCountList.index(truckMasterCountMax)
            print("Truck %s has the highest Master Count" %((truckMasterCountMaxIndex+1)))
